...
describe("Fluxo normal", () => {
    ...
    it("Um usuário deve ser cadastrado com e-mail e telefone único", async () => {
        ...
        expect(prismaMock.user.findUnique).toHaveBeenCalledWith({ //Verificando se a função de busca findUnique foi realizada para o e-mail
            where: { email: 'john.doe@example.com' },
        });
        expect(prismaMock.user.findUnique).toHaveBeenCalledWith({ //Verificando se a função de busca findUnique foi realizada para o telefone
...
describe("Fluxo de exceções", ()=>{
    ...
    it("Não deve ser possível criar um usuário que não possua uma senha", async () => {
        ...
        const res = { //Criando obejto response
            status: jest.fn().mockReturnThis(),
            json: jest.fn(),
        } as unknown as Response;
